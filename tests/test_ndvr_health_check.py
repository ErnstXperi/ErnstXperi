...
@pytest.mark.ndvr_health_check
@pytest.mark.notapplicable(not health_check)
class TestNdvrHealthCheck(object):
    """
    ...
    def test_ndvr_health_check(self, cn, mso=Settings.mso, clist=cl):
        ...
        show_details = self.api.record_currently_airing_shows(1, includeChannelNumbers=cn, health_check=True, row_filter=False)
        ...
        self.my_shows_page.select_and_start_recording_playback(self, self.home_page.convert_special_chars(show_details[0][0]))
